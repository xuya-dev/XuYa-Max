...
export class RoutePermissionValidator {
  ...
  static hasPermission(targetPath: string, menuList: AppRouteRecord[]): boolean {
    ...
    // 隐藏路由白名单：这些路由不在后端菜单里（isHide=true），但需要允许直接访问
    // （如 /system/user-center 个人中心、/system/message 消息中心等内部页）
    const HIDDEN_ROUTE_WHITELIST = [
      '/system/user-center',
      '/system/user/profile',
      '/system/message',
      '/system/dict/data'
    ]
    if (HIDDEN_ROUTE_WHITELIST.some((p) => targetPath === p || targetPath.startsWith(p + '/'))) {
      return true
    }
