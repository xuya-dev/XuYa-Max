<template>
  <div class="xuya-card h-82 p-5 mb-5 overflow-hidden max-lg:h-auto max-sm:mb-4">
    <div class="xuya-card-header pr-0">
      <div class="title">
        <h4>今日销售</h4>
        <p>销售总结</p>
      </div>
      <div class="flex-cc h-7.5 min-w-17 border border-g-300 rounded-lg text-g-500 c-p">
        <XuyaSvgIcon icon="ri:arrow-up-line" class="text-base mr-1.5" />
        ...
        <ElCol :span="6" :xs="24" v-for="(item, index) in salesData" :key="index">
          ...
          >
            <div class="size-12 rounded-lg flex-cc bg-theme/10">
              <XuyaSvgIcon :icon="item.icon" class="text-xl text-theme" />
            </div>

            <div class="max-sm:ml-4 mt-3.5 max-sm:mt-0 max-sm:text-end">
              <XuyaCountTo class="text-2xl font-medium" :target="item.value" :duration="1500" />
