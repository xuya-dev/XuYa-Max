...
<template>
  <div class="flex flex-col gap-4 pb-5">
    ...
    <!-- 表格区域 -->
    <ElCard class="flex-1 xuya-table-card" style="margin-top: 0">
      <template #header>
        <div class="flex-cb">
      ...
      >
        ...
        <template #operation="{ row }">
          <div class="flex">
            <XuyaButtonTable type="view" :row="row" @click="handleView(row)" />
            <XuyaButtonTable type="add" :row="row" @click="handleAdd()" />
            <XuyaButtonTable type="edit" :row="row" @click="handleEdit(row)" />
            <XuyaButtonTable type="delete" :row="row" @click="handleDelete(row)" />
          </div>
        </template>
...
<script setup lang="ts">
  ...
  // 搜索表单配置
  // 日期选择器有多种类型，具体可以查看 src/components/core/forms/xuya-search-bar/widget/xuya-search-date/README.md 文档
