...
export const upgradeLogList = ref<UpgradeLog[]>([
  {
    ...
    detail: [
      ...
      '修复：WebSocket 重连异常问题',
      '修复：特殊动态路由参数处理异常问题',
      '优化：XuyaForm、XuyaSearchBar 表单提交前增加数据清洗，避免无效字段提交到后端',
      '修复：XuyaTable 与 ElForm 组合使用时动态表单校验错误的问题',
  ...
  },
  {
    ...
    detail: [
      ...
      '新增：XuyaForm 和 XuyaSearchBar 组件的 label 支持自定义渲染函数，可实现带 tooltip 等复杂标签',
  ...
  },
  {
    ...
    detail: [
      ...
      '组件重构：重构 XuyaTextScroll 组件，提升性能与可维护性',
      '问题修复：修复 XuyaForm、XuyaSearchBar 自定义组件渲染异常',
      '功能增强：XuyaForm、XuyaSearchBar 新增 render 属性，支持自定义组件渲染',
      '功能增强：useTable hooks 新增 visible 属性，用于控制列默认是否显示',
      '响应式优化：优化 XuyaForm、XuyaSearchBar 栅格布局，适配多种屏幕尺寸',
  ...
  },
  {
    ...
    detail: [
      ...
      '优化 XuyaTable 顶部按钮换行无法自适应问题（示例：功能示例 / 左右布局表格）',
      'XuyaTable 分页组件选中样式优化',
      'XuyaTable 空状态高度默认撑满',
      'XuyaButtonMore 组件新增图标、颜色配置',
      'XuyaTableHeader 新增搜索按钮，用于控制顶部搜索栏的显示与隐藏',
      'XuyaSearchBar label 为空时不占空间',
      '表格操作栏拖拽禁止固定列拖拽',
      '角色管理页面接口对接、代码优化',
      '菜单管理页面优化',
      '优化设置中心滚动页面跟随滚动问题',
  ...
  },
  {
    ...
    date: '2025-08-17',
    detail: [
      '重构 XuyaSearchBar 组件，支持更多组件、表单校验等能力',
  ...
  },
  {
    ...
    detail: [
      ...
      '修复登录页面拖拽组件 XuyaDragVerify 宽度、颜色异常bug',
  ...
  },
  {
    ...
    detail: [
      ...
      '重构 XuyaBreadcrumb 面包屑导航组件，优化逻辑结构与样式',
      '优化 XuyaChatWindow 与 XuyaFastEnter 组件代码，提升可读性与性能',
      '重构 XuyaFireworksEffect 烟花效果组件，显著提升渲染性能与动画流畅度',
  ...
  },
  {
    ...
    detail: [
      'XuyaResultPage 组件重构',
      'XuyaTextScroll 组件代码优化',
      'XuyaException 组件增加类型提示',
      'XuyaCutterImg 组件样式优化、增加类型定义',
      'XuyaVideoPlayer 组件增加类型定义'
    ]
  },
  {
    version: 'v2.4.2.2',
