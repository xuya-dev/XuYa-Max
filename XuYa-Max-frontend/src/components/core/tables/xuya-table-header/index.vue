...
<template>
  <div class="flex-cb max-md:!block" id="xuya-table-header">
    <div class="flex-wrap">
      <slot name="left"></slot>
    </div>
    ...
    <div class="flex-c md:justify-end max-md:mt-3 max-sm:!hidden">
      ...
      >
        <XuyaSvgIcon icon="ri:search-line" :class="showSearchBar ? 'text-white' : 'text-g-700'" />
      </div>
      <div
      ...
      <ElDropdown v-if="shouldShow('size')" @command="handleTableSizeChange">
        <div class="button">
          <XuyaSvgIcon icon="ri:arrow-up-down-fill" />
        </div>
        <template #dropdown>
          <ElDropdownMenu>
            <div
      ...
      <div v-if="shouldShow('fullscreen')" class="button" @click="toggleFullScreen">
        <XuyaSvgIcon :icon="isFullScreen ? 'ri:fullscreen-exit-line' : 'ri:fullscreen-line'" />
      </div>
      ...
      <ElPopover v-if="shouldShow('columns')" placement="bottom" trigger="click">
        <template #reference>
          <div class="button">
            <XuyaSvgIcon icon="ri:align-right" />
          </div>
        </template>
        <div>
      ...
      <ElPopover v-if="shouldShow('settings')" placement="bottom" trigger="click">
        <template #reference>
          <div class="button">
            <XuyaSvgIcon icon="ri:settings-line" />
          </div>
        </template>
        <div>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'XuyaTableHeader' })
