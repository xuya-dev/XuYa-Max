...
<template>
  <div class="xuya-table" :class="{ 'is-empty': isEmpty }" :style="containerHeight">
...
<script setup lang="ts">
  ...
  /** XuyaTable 组件的 Props 接口 */
  interface XuyaTableProps extends TableProps<Record<string, any>> {
    /** 加载状态 */
    loading?: boolean
    /** 列渲染配置 */
    ...
    /** 空数据时显示的文本 */
    emptyText?: string
    /** 是否开启 XuyaTableHeader，解决表格高度自适应问题 */
    showTableHeader?: boolean
  }

  const props = withDefaults(defineProps<XuyaTableProps>(), {
    columns: () => [],
    fit: true,
    showHeader: true,
    stripe: undefined,
  ...
  const mergedTableProps = computed(() => ({
    ...
    // 默认表头与内容居中：通过 class 注入，列级 align 仍可覆盖
    headerCellClassName: props.headerCellClassName ?? 'xuya-table-cell--center',
    cellClassName: props.cellClassName ?? 'xuya-table-cell--center',
    // Element Plus 默认值为 true，未显式传入时不应被 XuyaTable 覆盖成 false。
  ...
  const findTableHeader = () => {
    ...
    const tableHeader = document.getElementById('xuya-table-header')
    if (tableHeader) {
