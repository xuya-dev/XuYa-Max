...
<template>
  <div>
    <ElDropdown v-if="hasAnyAuthItem">
      <XuyaIconButton icon="ri:more-2-fill" class="!size-8 bg-g-200 dark:bg-g-300/45 text-sm" />
      <template #dropdown>
        <ElDropdownMenu>
          <template v-for="item in list" :key="item.key">
            ...
            >
              <div class="flex-c gap-2" :style="{ color: item.color }">
                <XuyaSvgIcon v-if="item.icon" :icon="item.icon" />
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'XuyaButtonMore' })
