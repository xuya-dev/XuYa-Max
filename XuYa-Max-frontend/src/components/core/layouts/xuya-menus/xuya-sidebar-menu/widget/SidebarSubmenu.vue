  ...
  <template v-for="(item, index) in filteredMenuItems" :key="getUniqueKey(item, index)">
    <ElSubMenu v-if="hasChildren(item)" :index="item.path || item.meta.title" :level="level">
      <template #title>
        ...
        <div v-if="item.meta.showBadge" class="xuya-badge" style="right: 10px" />
      </template>
    ...
    >
      ...
      <template #title>
        ...
        <div v-if="item.meta.showBadge" class="xuya-badge" />
        <div v-if="item.meta.showTextBadge && (level > 0 || menuOpen)" class="xuya-text-badge">
