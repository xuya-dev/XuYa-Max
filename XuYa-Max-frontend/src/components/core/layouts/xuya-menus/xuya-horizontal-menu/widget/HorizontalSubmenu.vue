<template>
  <ElSubMenu v-if="hasChildren" :index="item.path || item.meta.title" class="!p-0">
    <template #title>
      <XuyaSvgIcon :icon="item.meta.icon" :color="theme?.iconColor" class="mr-1 text-lg" />
      <span class="text-md">{{ formatMenuTitle(item.meta.title) }}</span>
      <div v-if="item.meta.showBadge" class="xuya-badge xuya-badge-horizontal" />
      <div v-if="item.meta.showTextBadge" class="xuya-text-badge">
  ...
  >
    ...
    />
    <div v-if="item.meta.showTextBadge && level !== 0" class="xuya-text-badge">
      {{ item.meta.showTextBadge }}
    </div>
  </ElMenuItem>
</template>
