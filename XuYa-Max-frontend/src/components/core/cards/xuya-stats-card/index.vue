...
<template>
  <div
    class="xuya-card h-32 flex-c px-5 transition-transform duration-200 hover:-translate-y-0.5"
    :class="boxStyle"
  >
    <div v-if="icon" class="mr-4 size-11 flex-cc rounded-lg text-xl text-white" :class="iconStyle">
      <XuyaSvgIcon :icon="icon"></XuyaSvgIcon>
    </div>
    <div class="flex-1">
    ...
    <div v-if="showArrow">
      <XuyaSvgIcon icon="ri:arrow-right-s-line" class="text-xl text-g-500" />
    </div>
  </div>
</template>

<script setup lang="ts">
  defineOptions({ name: 'XuyaStatsCard' })
