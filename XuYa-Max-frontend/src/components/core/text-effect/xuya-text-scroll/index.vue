...
<template>
  ...
  >
    <div class="flex-cc absolute left-0 h-full w-9 z-10" :style="{ backgroundColor: bgColor }">
      <XuyaSvgIcon icon="ri:volume-down-line" class="text-lg" />
    </div>
    ...
    <div
      ...
      @click="handleClose"
    >
      <XuyaSvgIcon icon="ri:close-fill" class="text-lg" />
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const bgColor = computed(
    () =>
      `color-mix(in oklch, var(--color-${props.type}) ${isDark.value ? '25' : '10'}%, var(--xuya-color))`
  )
