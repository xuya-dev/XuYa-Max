    ...
    <div class="flex-cc gap-1.5 mr-2 max-sm:mr-5">
      <div class="color-picker-expandable relative flex-c max-sm:!hidden">
        ...
        >
          ...
          >
            <XuyaSvgIcon v-if="color === systemThemeColor" icon="ri:check-fill" class="text-white" />
          </div>
        </div>
          ...
          <ElDropdownMenu>
            <div v-for="lang in languageOptions" :key="lang.value" class="lang-btn-item">
              ...
              >
                ...
                <XuyaSvgIcon icon="ri:check-fill" class="text-base" v-if="locale === lang.value" />
              </ElDropdownItem>
            </div>
          </ElDropdownMenu>
        </template>
...
<style scoped>
  .color-dots {
    ...
    box-shadow: 0 2px 12px var(--xuya-gray-300);
    transition:
      opacity 0.3s ease,
      transform 0.3s ease;
  ...
  .color-picker-expandable:hover .palette-btn :deep(.xuya-svg-icon) {
    color: v-bind(color);
  }
</style>
