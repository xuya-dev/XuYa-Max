...
description: 复杂前端页面增强专家。用于修改 XuYa-Max-frontend 中已存在的列表页、详情弹窗、富文本页、SSE 消息联动页，强调增量修改、保留 useTable/XuyaSearchBar 既有交互、维护富文本安全渲染与 OSS 占位解析。
---
...
2. 增量修改，不重写整页。
3. 保留已有 useTable 配置、XuyaSearchBar 字段、XuyaTableHeader 列显隐、XuyaDialog 弹窗、权限指令（`v-auth`）、字典渲染（`XuyaDictTag`）。
...
- 右上角通知面板：`src/components/core/layouts/xuya-notification/index.vue`
...
- 是否破坏了原页面的 useTable 配置或 XuyaTable 列定义
- 是否误删了 `v-auth` 权限指令或 `XuyaDictTag` 字典渲染
