...
description: 在 XuYa-Max-frontend（Vue 3 + Element Plus 前端 + Spring Boot 后端）中按真实 Vue 3 + TypeScript + Element Plus + Tailwind v4 + useTable + ArtTable/ArtSearchBar/ArtDialog 代码风格生成或修改 CRUD 页面、API、types、组件接入和样式壳。用于新增或修改标准 CRUD 列表页、树表页、系统管理页、监控页、详情弹窗、富文本页、SSE 消息联动，补齐与后端接口对应的 src/api、types 和 src/views 代码；触发后应先读取适用 references，再阅读目标模块真实代码。
---
...
5. 增强已有页面时只做增量修改，保留原页面的 useTable 配置、ArtSearchBar 字段、ArtTableHeader 列显隐、ArtDialog 弹窗、权限指令、字典渲染。
6. 修改完成后按影响范围运行验证：
...
- 组件自动注册（unplugin-vue-components）：`ArtTable/ArtTableHeader/ArtSearchBar/ArtButtonTable/ArtDictTag/ElCard/ElButton/ElDialog` 等模板里直接用，无需 import；但 `h()` 渲染时要显式 import。
...
- 字典：`useDict('sys_xxx')` 返回 reactive 对象，用 `ArtDictTag` 渲染。
- Tailwind 语义色：`bg-theme/text-theme/success/error/warning/info/g-100..g-900`（在 `tailwind.css` 注册）；自定义类 `flex-c/flex-b/flex-cc/c-p/art-full-height/art-table-card`。
...
- 树筛选面板（用 `ArtTreePanel`）
- 导入弹窗（用 `ArtExcelImport`）
- 列显隐（`ArtTableHeader` 自带）
...
参考 `src/views/system/dict/data/index.vue`：搜索栏直接内联 ElForm（不用 ArtSearchBar），弹窗内联。
...
- 列表页用 `useTable + ArtTable + ArtTableHeader + ArtSearchBar` 模式，CRUD 流程闭环完整。
- 复杂页面保留原有交互能力和业务约束。
...
- 字典渲染是否用 `ArtDictTag`（h 函数里要 import）。
